...
- [ ] A scroll cue is rendered as the bar the reference shows - `overflow-y: scroll` for an always-visible track, or the styled element for a custom indicator - not left to `overflow-y: auto`, which renders nothing when the counted content stops at the frame's edge. No rows were added to make a bar appear.
